...
- индексы #emph("IDX_STUDENTS_NAME") и #emph("IDX_STUDENTS_EMAIL"), отсутствующие в #emph("PROD_USER");
- процедуры #emph("GET_STUDENT_COUNT") и #emph("ADD_STUDENT"), отсутствующие в #emph("PROD_USER");
- функции #emph("GET_DEPT_NAME") и #emph("GET_STUDENT_EMAIL"), отсутствующие в #emph("PROD_USER");
- пакет #emph("UNIVERSITY_PKG"), отсутствующий в #emph("PROD_USER").

В схеме #emph("PROD_USER") были созданы устаревшие объекты для тестирования удаления:
- таблица #emph("OLD_TABLE");
- индекс #emph("IDX_OLD").
...
== Сравнение процедур, функций, индексов и пакетов

Для сравнения других объектов базы данных используются представления #emph("ALL_OBJECTS") и #emph("ALL_PROCEDURES"). Функция определяет объекты, отсутствующие в Prod или существующие только в Prod:

#stp2024.listing[Сравнение процедур и функций][
  ```
  FOR r IN (
      ...
      -- Объект MISSING - нужен CREATE
  END LOOP;
  ```
]

Для генерации DDL-скриптов используется #emph("DBMS_METADATA.GET_DDL"), который возвращает полный текст объекта. Имя схемы из результата удаляется для возможности создания в Prod.

== Генерация DDL-скриптов и удаление объектов

Задание 3 подразумевает не только поиск различий, но и генерацию готовых SQL-скриптов для синхронизации:

- #emph("MISSING") -- объект отсутствует в Prod, нужно создать: генерируется #emph("CREATE TABLE"), #emph("CREATE INDEX"), #emph("CREATE OR REPLACE PROCEDURE/FUNCTION/PACKAGE");
- #emph("DIFFERENT") -- структура объекта отличается, нужно изменить: генерируется #emph("ALTER TABLE ADD COLUMN") для добавления отсутствующих колонок;
- #emph("EXTRA_IN_PROD") -- объект есть в Prod, но отсутствует в Dev, нужно удалить: генерируется #emph("DROP TABLE"), #emph("DROP INDEX") и т.д.
...
#figure(
  image("img/compare_result.png", width: 100%),
  caption: [Результат сравнения схем DEV_USER и PROD_USER]
) <fig:result>

Таблицы отсортированы с учётом зависимостей через #emph("FOREIGN KEY"): сначала #emph("STUDENTS") (зависит от #emph("DEPARTMENTS")), затем #emph("ENROLLMENTS") (зависит от #emph("STUDENTS") и #emph("COURSES")), затем #emph("INSTRUCTORS"). Объекты с типом #emph("EXTRA_IN_PROD") выводятся в конце для удаления.

Для синхронизации PROD с DEV необходимо выполнить DDL-скрипты для объектов со статусом #emph("MISSING") и #emph("DIFFERENT"), а затем удалить объекты со статусом #emph("EXTRA_IN_PROD").
